...
회원가입한 사용자가 여러 사주 프로필을 등록하고, 선택한 프로필의 MVP 만세력 계산 기반 사주 풀이를 바탕으로 AI 상담을 진행하는 Spring Boot 백엔드 MVP입니다.

## 핵심 기능

- JWT 기반 회원가입/로그인
- 사용자별 다중 사주 프로필 생성, 목록, 상세, 삭제
- 사주팔자 년주/월주/일주/시주, 오행/음양 분포를 포함한 MVP 만세력 기반 분석 결과 생성 및 DB 저장
...
현재 분석은 전문 사주 서비스 수준의 완전한 만세력이 아니라 MVP용 기본 만세력 계산과 rule-based 문구 생성을 결합한 구조입니다.

- 년주, 월주, 일주, 시주를 계산해 사주팔자를 표시합니다.
- 천간/지지를 바탕으로 오행 분포와 음양 분포를 계산합니다.
- 생월을 기준으로 봄, 여름, 가을, 겨울의 계절 성향을 보조 판단합니다.
- 출생시간을 기준으로 새벽, 오전, 오후, 저녁/밤의 시간대 에너지를 보조 판단합니다.
- 양력/음력 선택값에 따라 현실적 흐름 또는 전통적/관계적 흐름의 문구를 조정합니다.
- 음력/윤달 값은 저장, 응답, 캐시 키에 반영하지만 현재 버전은 검증된 음양력 변환을 수행하지 않습니다.
- 성별은 현재 단계에서 차별적 해석에 사용하지 않고 프로필 정보로만 유지합니다.
- 향후 실제 음양력 변환, 24절기, 십성, 대운/세운 계산 모듈로 교체할 수 있도록 `SajuAnalysisService`와 `ManseCalendarService`로 분리되어 있습니다.

자세한 계산 기준과 한계는 [docs/MANSE_CALENDAR.md](docs/MANSE_CALENDAR.md)를 참고하세요.

분석 캐시 키는 만세력 계산 버전 반영을 위해 `saju:analysis:v3:manse` 형식을 사용합니다. 기존 `saju:profile:v2` 캐시는 새 분석 로직에서 사용하지 않습니다.
...
테스트에는 양력/음력/윤달 프로필 생성, 사주팔자 null 여부, 오행/음양 분포 합계, Redis 분석 캐시 키 버전, 프로필 삭제 시 상담 기록 삭제, AI 상담 요청 제한 검증이 포함됩니다.
